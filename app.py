...
def thank_you():
    try:
        ...
        # Validate transaction_id format for security (allow only alphanumeric, dash, underscore)
        import re
        if transaction_id_raw and re.match(r'^[A-Za-z0-9_-]+$', transaction_id_raw):
            validated_transaction_id = transaction_id_raw
        else:
            validated_transaction_id = ''
            if transaction_id_raw:
                app.logger.warning(f"[PROD] Invalid transaction_id format rejected: {transaction_id_raw}")
...
def create_pix_payment():
    try:
        ...
        # Usar o gateway configurado (4M Pagamentos, For4, ou NovaEra)
        from payment_gateway import get_payment_gateway
        
        try:
            # Usar gateway configurado para pagamento da taxa regional
            api = get_payment_gateway()
...
def verificar_pagamento():
    try:
        ...
        # Usar o gateway configurado para verificação de status
        from payment_gateway import get_payment_gateway
        api = get_payment_gateway()
...
def pagamento_encceja():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            demo_payment_data = {
                'id': '4M123456',  # ID válido para 4M Pagamentos
